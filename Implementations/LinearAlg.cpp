#include <stdio.h>
#include <string.h>
#include <string>
#include <vector>
#include <queue>
#include <set>
#include <bitset>
#include <map>
#include <algorithm>

using namespace std;

//enable debugging
#define debug 1

// Variables count limit
#define NV 100

typedef pair<int,int> ii;
typedef vector<int> vi;
typedef bitset<NV> bs;
typedef vector<bs> vbs;

int vCnt;
map<vi, int> vti; // maps vectors which represent a string to an int
map<int, vi> itv;

vi variableInstances; // the number of instances of each variable

void enumerateStrings(int n, vi curV){
    if(curV.size()==n){
        vti[curV]=vCnt;
        itv[vCnt++]=curV;
        return;
    }
    int curI = curV.size();
    for(int i=0; i<variableInstances[curI]; i++){
        curV.push_back(i);
        enumerateStrings(n, curV);
        curV.pop_back();
    }
}

bool checkConstraints(vi remStrings, int n, vbs A, vi f){
    sort(remStrings.begin(), remStrings.end());
    vector<vi> strings;
    for(int i=0; i<itv.size(); i++){
        vi cur = itv[i];
        if(binary_search(remStrings.begin(), remStrings.end(), i)){
            continue;
        }
        strings.push_back(cur);
    }

    for(int i=0; i<A.size(); i++){
        if(A[i].size()==1) continue;
        set<vi> s;
        for(int j=0; j<strings.size(); j++){
            vi curV;
            for(int k=0; k<n; k++){
                if(!A[i][k]) continue;
                curV.push_back(strings[j][k]);
            }
            s.insert(curV);
            if(s.size()>f[i]) return false;
        }
    }
    return true;
}

void removeString(vi curString, vi varMap, bs rule, int n, int varOrig, int lastVar, int &leftPatterns, vector<set<int> > &usedInstances, set<int> &remStrings){
    if(leftPatterns==0) return;
    if(varOrig==n){
        if(debug){
            printf("%s::%d:: ", rule.to_string().substr(rule.size()-n).c_str(), remStrings.size());
            for(int i=0; i<n; i++){
                printf("V%d: %d, ", i, curString[i]+1);
            }
            printf("\n");
        }
        remStrings.insert(vti[curString]);
        return;
    }
    int var = varMap[varOrig];
    if(rule[var]){
        int val=0;
        while(val<variableInstances[var] && leftPatterns>0){
            usedInstances[var].insert(val);
            curString[var]=val;
            removeString(curString, varMap, rule, n, varOrig+1, lastVar, leftPatterns, usedInstances, remStrings);
            val++;
            if(var==lastVar) leftPatterns--;
        }
    }else{
        for(int i=0; i<variableInstances[var]; i++){
            curString[var]=i;
            removeString(curString, varMap, rule, n, varOrig+1, lastVar, leftPatterns, usedInstances, remStrings);
        }
    }
}

int solve(int n, vbs A, vi f){
    vCnt=0;
    vti.clear();
    itv.clear();
    enumerateStrings(n, vi());

    // construct DAG
    vector<vi> graph(A.size(), vi());
    vector<vi> graphT(A.size(), vi());

    for(int i=0; i<A.size(); i++){
        for(int j=0; j<A.size(); j++){
            if(i==j) continue;
            bool subset=1;
            for(int k=0; k<n; k++){
                if(A[i][k] && !A[j][k]){
                    subset=0;
                    break;
                }
            }
            if(subset){
                graph[j].push_back(i);
                graphT[i].push_back(j);
            }
        }
    }

    vi nStrings(A.size());
    // init total number of strings
    for(int i=0; i<A.size(); i++){
        nStrings[i]=1;
        for(int j=0; j<n; j++){
            if(A[i][j]) nStrings[i]*=variableInstances[j];
        }
    }

    // generate levels
    vector<set<int> > levels;
    levels.push_back(set<int>());
    for(int i=0; i<A.size(); i++){
        if(graphT[i].size()==0){
            levels[0].insert(i);
        }
    }
    for(int i=0; i<levels.size(); i++){
        levels.push_back(set<int>());
        for(set<int>::iterator it=levels[i].begin(); it!=levels[i].end() ; it++){
            for(int k=0; k<graph[*it].size(); k++){
                levels[i+1].insert(graph[*it][k]);
            }
        }
        if(levels[i+1].size()==0) break;
    }
    levels.pop_back();

    vector<set<int> > usedInstances(n);
    set<int> remStrings;

    for(int i=levels.size()-2; i>=0; i--){
        for(set<int>::iterator it=levels[i].begin(); it!=levels[i].end(); it++){
            int node = *it;
            if(A[node].count()==1){
                continue;
            }

            int left = nStrings[node]-f[node];

            vector<ii> vtmp;
            for(int k=0; k<n; k++) if(A[node][k]) vtmp.push_back(ii(usedInstances[k].size(), k));
            sort(vtmp.rbegin(), vtmp.rend());

            int lastVar=vtmp.back().second;
            vi varMap;
            for(int k=0; k<vtmp.size(); k++) varMap.push_back(vtmp[k].second);
            for(int k=0; k<n; k++) if(!A[node][k]) varMap.push_back(k);

            removeString(vi(n), varMap, A[node], n, 0, lastVar, left, usedInstances, remStrings);
        }
    }
    if(checkConstraints(vi(remStrings.begin(), remStrings.end()), n, A, f)) return vti.size()-remStrings.size();
    return -1;
}

int test0(){
    int n=2;
    vbs A;

    bs A1(string("01"));
    A.push_back(A1);

    bs A2(string("10"));
    A.push_back(A2);

    bs A3(string("11"));
    A.push_back(A3);

    vi f;
    f.push_back(3);
    f.push_back(4);
    f.push_back(7);

    variableInstances.clear();
    variableInstances.push_back(3);
    variableInstances.push_back(4);

    return solve(n, A, f);
}

int test1(){
    int n=3;

    vbs A;

    bs A1(string("001"));
    A.push_back(A1);

    bs A2(string("010"));
    A.push_back(A2);

    bs A3(string("100"));
    A.push_back(A3);

    bs A4(string("011"));
    A.push_back(A4);

    bs A5(string("101"));
    A.push_back(A5);

    bs A6(string("110"));
    A.push_back(A6);

    vi f;

    f.push_back(4);
    f.push_back(6);
    f.push_back(3);
    f.push_back(8);
    f.push_back(8);
    f.push_back(6);

    variableInstances.clear();
    variableInstances.push_back(4);
    variableInstances.push_back(6);
    variableInstances.push_back(3);

    return solve(n,A,f);
}

int test2(){
    int n=5;
    // v w z y x
    vbs A;
    bs A1(string("00001"));
    A.push_back(A1);

    bs A2(string("00010"));
    A.push_back(A2);

    bs A3(string("00100"));
    A.push_back(A3);

    bs A4(string("01000"));
    A.push_back(A4);

    bs A5(string("10000"));
    A.push_back(A5);

    bs A6(string("00110"));
    A.push_back(A6);

    bs A7(string("10100"));
    A.push_back(A7);

    bs A8(string("01100"));
    A.push_back(A8);

    bs A9(string("00011"));
    A.push_back(A9);

    bs A10(string("00101"));
    A.push_back(A10);

    bs A11(string("01011"));
    A.push_back(A11);

    bs A12(string("00111"));
    A.push_back(A12);

    vi f;
    f.push_back(3);
    f.push_back(2);
    f.push_back(3);
    f.push_back(2);
    f.push_back(3);

    f.push_back(3);
    f.push_back(7);
    f.push_back(4);
    f.push_back(4);
    f.push_back(6);
    f.push_back(6);
    f.push_back(12);

    variableInstances.clear();
    variableInstances.push_back(3);
    variableInstances.push_back(2);
    variableInstances.push_back(3);
    variableInstances.push_back(2);
    variableInstances.push_back(3);

    return solve(n,A,f);
}

int test3(){
    int n=3;

    vbs A;

    bs A1(string("001"));
    A.push_back(A1);

    bs A2(string("010"));
    A.push_back(A2);

    bs A3(string("100"));
    A.push_back(A3);

    bs A4(string("011"));
    A.push_back(A4);

    bs A5(string("101"));
    A.push_back(A5);

    bs A6(string("110"));
    A.push_back(A6);

    vi f;

    f.push_back(3);
    f.push_back(2);
    f.push_back(2);
    f.push_back(8);
    f.push_back(8);
    f.push_back(6);

    variableInstances.clear();
    variableInstances.push_back(3);
    variableInstances.push_back(2);
    variableInstances.push_back(2);

    return solve(n, A, f);
}

int test4(){
    int n=5;
    // v w z y x
    vbs A;
    bs A1(string("00001"));
    A.push_back(A1);

    bs A2(string("00010"));
    A.push_back(A2);

    bs A3(string("00100"));
    A.push_back(A3);

    bs A4(string("01000"));
    A.push_back(A4);

    bs A5(string("10000"));
    A.push_back(A5);

    bs A6(string("00110"));
    A.push_back(A6);

    bs A7(string("10100"));
    A.push_back(A7);

    bs A8(string("01100"));
    A.push_back(A8);

    bs A9(string("00011"));
    A.push_back(A9);

    bs A10(string("00101"));
    A.push_back(A10);

    bs A11(string("01011"));
    A.push_back(A11);

    bs A12(string("00111"));
    A.push_back(A12);

    vi f;
    f.push_back(3);
    f.push_back(1);
    f.push_back(1);
    f.push_back(1);
    f.push_back(4);

    f.push_back(3);
    f.push_back(7);
    f.push_back(4);
    f.push_back(4);
    f.push_back(6);
    f.push_back(6);
    f.push_back(12);

    variableInstances.clear();
    variableInstances.push_back(3);
    variableInstances.push_back(1);
    variableInstances.push_back(1);
    variableInstances.push_back(1);
    variableInstances.push_back(4);

    return solve(n,A,f);
}

int test5(){
    int n=3;

    vbs A;

    bs A1(string("001"));
    A.push_back(A1);

    bs A2(string("010"));
    A.push_back(A2);

    bs A3(string("100"));
    A.push_back(A3);

    bs A4(string("011"));
    A.push_back(A4);

    bs A5(string("101"));
    A.push_back(A5);

    bs A6(string("110"));
    A.push_back(A6);

    vi f;

    f.push_back(2);
    f.push_back(4);
    f.push_back(2);
    f.push_back(6);
    f.push_back(3);
    f.push_back(5);

    variableInstances.clear();
    variableInstances.push_back(2);
    variableInstances.push_back(4);
    variableInstances.push_back(2);

    return solve(n,A,f);
}

int main(){
    printf("Test 0: %d\n", test0());
    printf("Test 1: %d\n", test1());
    printf("Test 2: %d\n", test2());
    printf("Test 3: %d\n", test3());
    printf("Test 4: %d\n", test4());
    printf("Test 5: %d\n", test5());

    return 0;
}
